...
export default function MasterStorageOptionsPage() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      const data = await masterApi.get('/master/storage-options');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modal.type === 'create') {
        await masterApi.post('/master/storage-options', body);
      } else {
        await masterApi.put(`/master/storage-options/${modal.item.id}`, body);
      }
      closeModal();
      load();
    } catch (e) {
  ...
  const handleDelete = async (row) => {
    if (!confirm('Delete this storage option?')) return;
    try {
      await masterApi.delete(`/master/storage-options/${row.id}`);
      load();
    } catch (e) {
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-slate-100">Storage Options</h1>
        <button
          type="button"
          onClick={openCreate}
      ...
      <p className="text-admin-muted text-sm mb-4">
        Used in mobile app dropdowns (GET /api/master/storage-options).
      </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-md rounded-xl bg-admin-card border border-admin-border p-6">
            <h2 className="text-lg font-medium text-slate-100 mb-4">
              {modal.type === 'create' ? 'New storage option' : 'Edit storage option'}
            </h2>
